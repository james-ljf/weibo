package com.demo.weibo.common.util.id;
...
public class IdGenerator {
    ...
    static{
        try {
            String Id = IpUtils.getHostIp();
            String name = IpUtils.getHostName();
//            workerId = NetUtil.ipv4ToLong(NetUtil.getLocalhostStr()) % 32;
            log.info("当前机器 workerIp: {}", Id);
            log.info("当前机器 workerhostname: {}", name);
        } catch (Exception e) {
